...
| Versi          | 0.47 (Laporan & Settings sinkron — kop cetak lengkap, Company Profile via server inventory, sidebar accordion, laporan sub-menu & title dinamis) |
...
| M3-7 | Retur & koreksi | Reuse `retur-penjualan` (sudah tersedia) — workflow kasir menyusul | Retur mengembalikan stok sesuai behavior | ✅ (M3-FIX v11: engine behavior-aware + `sumber=pos` + blokir void; UI kasir: retur/refund order F&B ✅ v0.46, retur retail reguler = T13 backlog) |
...
| Fitur | Catatan |
|-------|---------|
| Recipe Engine | ✅ M6.2 (BOM engine + varian); lanjutan V2: Modifier, Topping, Production |
| Kitchen Display System (KDS) | ✅ M6.4 (role CHEF + pembatalan per-item, v0.46) |
| Table & Order + QR Ordering | ✅ M6.3 + M6.5 (QR meja, order meja, `/m/` customer, v0.46) |
...
> - `docs/pos/pos_execution_3.md` — **Log Revisi — Bagian 3**: entry v0.46
>   (2026-08-17 — **F&B Customer Ordering V1**: M6.3 Table & Order Management,
>   M6.4 Kitchen Display System, M6.5 Customer QR Ordering, Payment Proof,
>   Role-Based Notification Center, Web Push, WhatsApp Notify, Company Payment
>   Settings, Company Profile fix, Shift Refund, PWA — entry lengkap di sana)
>
> Entry terbaru (v0.44 – v0.45) dicantumkan **penuh** di bawah ini; v0.46+ di `pos_execution_3.md`.

| Versi | Tanggal | Penulis | Deskripsi |
|-------|---------|---------|-----------|
| 0.46 | 2026-08-17 | Freebuff | **F&B Customer Ordering V1 (SP-029 POS V1 — M6.3 + M6.4 + M6.5)** — M6.3 Table & Order Management (QR Meja permanen + Order Meja embedded kasir + konfirmasi bayar manual + refund), M6.4 Kitchen Display System (role CHEF baru, pembatalan per-item), M6.5 Customer QR Ordering (`/m/` tanpa login, server-side pricing, Web Push "pesanan siap", checkout via WhatsApp), Payment Proof (upload bukti QRIS/Transfer + verifikasi kasir manual), Role-Based Notification bell (kitchen order_new / cashier payment_proof & order_cancelled), Web Push (VAPID + subscription per order), WhatsApp Notify (Sidobe — Settings → Konfigurasi WA + Test Koneksi), Company Payment Settings (QRIS company + rekening maks 3), Company Profile via server POS (fix 401 audience isolation), Shift Refund (pengurang penjualan & expectedCash), PWA (manifest kasir/QR/tablet). **Full suite 1099/1099 (61 files)**, build pos+inventory+console ✓. Entry lengkap: `docs/pos/pos_execution_3.md`. |
