...
import { Modal, Toast } from "../../index.js";
...
export function PlatformDashboardModule({
        ...
        return `
            <div id="platform-dashboard" class="platform-dashboard">
                ...
                <div id="pd-nav" class="pd-nav">
                    ...
                    <button class="pd-nav-btn" data-view="superadmins">👤 Super Admin</button>
    ...
    function initPlatformDashboard() {
        ...
        // Navigation tabs
        document.querySelectorAll(".pd-nav-btn").forEach(btn => {
            btn.addEventListener("click", () => {
                document.querySelectorAll(".pd-nav-btn").forEach(b => b.classList.remove("active"));
    ...
    function showAppsView() {
        ...
        content.innerHTML = `
            <div class="pd-view">
                ...
                <p class="pd-view-desc">Pilih aplikasi untuk melihat perusahaan yang memiliki akses, lalu Login As Admin Perusahaan.</p>
                <div class="pd-apps-grid">
                    ${apps.map(app => `
                        <div class="pd-app-card" data-app="${app.slug}">
                            <div class="pd-app-icon">${app.icon || "📱"}</div>
        ...
        content.querySelectorAll(".pd-app-card").forEach(card => {
            card.addEventListener("click", () => showCompaniesView(card.dataset.app));
        });
    }
            ...
            content.innerHTML = `
                <div class="pd-view">
                    <div class="pd-view-back">
                        ...
                        <h2 class="pd-view-title">${app.icon || ""} ${app.name}</h2>
        ...
        try {
            ...
            content.innerHTML = `
                <div class="pd-view">
                    ...
                    <div class="pd-mgmt-table-wrapper">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="pd-mgmt-actions">
                                                ...
                                                <button class="pd-action-btn pd-action-loginas" data-action="loginas" data-code="${c.code}" data-name="${esc(c.name || c.code)}" title="Login As Admin Perusahaan">🔑 Login As</button>
    ...
    function showCompanyForm(title, item, onSubmit) {
        ...
        const typeOptions = (COMPANY_TYPES || ["PT", "CV", "Perorangan", "BUMDes", "Koperasi", "Pesantren", "Pemerintah", "Lainnya"])
        ...
        // Auto-fill admin username when company code changes (only for new company)
        if (!isEdit) {
            const codeInput = document.getElementById("f-code");
            const adminUserInput = document.getElementById("f-admin-username");
            if (codeInput && adminUserInput) {
                codeInput.addEventListener("input", () => {
                    const val = codeInput.value.trim().toLowerCase().replace(/[^a-z0-9]/g, '_');
                    if (val && !adminUserInput.dataset.userEdited) {
    ...
    function startImpersonation(appSlug, companyCode, companyName) {
        ...
        if (appSlug === "inventory") {
            sessionStorage.setItem("smart_impersonation", JSON.stringify({
                session: { superAdminId: superAdmin.id, superAdminName: superAdmin.name,
    ...
    async function showSuperAdminManagement() {
        ...
        const content = document.getElementById("pd-content");
        if (!content) return;
        content.innerHTML = `<div class="pd-loading">Memuat daftar Super Admin...</div>`;
        ...
        try {
            ...
            content.innerHTML = `
                <div class="pd-view">
                    <div class="pd-view-header">
                        <div>
                            <h2 class="pd-view-title">👤 Kelola Super Admin</h2>
                            <p class="pd-view-desc">Total ${saState.users.length} akun Super Admin platform</p>
                        </div>
                        <button class="pd-add-company-btn" id="pd-add-superadmin">➕ Tambah Super Admin</button>
                    </div>
                    <div class="pd-mgmt-table-wrapper">
                        <table class="pd-mgmt-table">
                            <thead>
                            ...
                            <tbody>
                                ${saState.users.map(u => `
                                    <tr>
                                        ...
                                        <td>
                                            <div class="pd-mgmt-actions">
                                                <button class="pd-action-btn pd-action-edit" data-action="sa-edit" data-id="${u._id}" data-username="${esc(u.username)}" title="Edit Super Admin">✏️ Edit</button>
                                                <button class="pd-action-btn pd-action-disable" data-action="sa-delete" data-id="${u._id}" data-username="${esc(u.username)}" title="Hapus Super Admin">🗑️ Hapus</button>
                                            </div>
                                        </td>
                                    </tr>
                                `).join("")}
                                ${saState.users.length === 0 ? '<tr><td colspan="5" class="pd-empty">Belum ada Super Admin</td></tr>' : ""}
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    function openAddSuperAdmin() {
        ...
        const modal = Modal({
            open: true, title: "➕ Tambah Super Admin",
            content: `
                <div class="pd-form">
                    <div class="form-grid">
                        <div class="form-group">
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" id="f-name" placeholder="Nama Super Admin" required />
                        </div>
                        <div class="form-group full-width">
                            <label>Email</label>
        ...
        document.getElementById("f-submit")?.addEventListener("click", async () => {
            ...
            try {
                ...
                showToast("success", `Super Admin "${name}" berhasil ditambahkan`);
                removeModal();
    ...
    async function openEditSuperAdmin(id) {
        try {
            ...
            const modal = Modal({
                open: true, title: `✏️ Edit Super Admin — ${user.username}`,
                content: `
                    <div class="pd-form">
                        <div class="form-grid">
                            <div class="form-group">
            ...
            document.getElementById("f-submit")?.addEventListener("click", async () => {
                ...
                try {
                    ...
                    showToast("success", `Super Admin "${user.username}" berhasil diperbarui`);
                    removeModal();
                    showSuperAdminManagement();
                } catch (err) {
                    showToast("danger", "Gagal: " + err.message);
    ...
    async function deleteSuperAdminAction(id, username) {
        ...
        const modal = Modal({
            ...
            content: `<p>Hapus Super Admin <strong>${esc(username)}</strong>?</p>
        ...
        document.getElementById("f-confirm")?.addEventListener("click", async () => {
            ...
            if (deleted) {
                showToast("success", `Super Admin "${username}" berhasil dihapus`);
            } else {
                showToast("danger", "Gagal menghapus Super Admin");
            }
            removeModal();
            showSuperAdminManagement();
        });
...
.pd-app-card {
    ...
    border: 1px solid #e2e8f0; cursor: pointer; transition: all 0.2s; text-align: center;
}
.pd-app-card:hover { border-color: #4f46e5; box-shadow: 0 4px 16px rgba(79,70,229,0.1); transform: translateY(-2px); }
.pd-app-icon { font-size: 2.5rem; margin-bottom: 0.75rem; }
