 ...
 *
 * @module @smart/ui/modules/barang
 */
...
export function BarangModule(services) {
    ...
    async function openForm(mode, id = null) {
        ...
        try { rakOptions = (await listRak({ page: 1, limit: 999 })).data.map(r => r.nama); } catch {}
        try { gudangOptions = (await listWarehouse({ page: 1, limit: 999 })).data.map(g => g.nama); } catch {}
    ...
    function renderModal(title, contentHTML, isEdit, editId) {
        ...
        document.getElementById("f-rak")?.addEventListener("change", async (e) => { if (e.target.value === "__add_rak__") { e.target.value = ""; await showQuickAdd("rak", e.target); } });
    ...
    async function showQuickAdd(fieldType, selectEl) {
        ...
        document.body.appendChild(overlay);
        setTimeout(() => document.getElementById("qa-nama")?.focus(), 150);
...
.sm-card .card-body { display:flex; flex-direction:column; gap:0.25rem; margin-bottom:0.5rem; }
.sm-card .card-price-row, .sm-card .card-stok-row { display:flex; justify-content:space-between; align-items:center; }
