...
router.get("/", async (req, res) => {
    try {
        ...
        const data = await Rak.find(query)
            .sort({ createdAt: -1 })
            .skip((page - 1) * limit)
            .limit(limit);
...
router.get("/check-kode/:kode", async (req, res) => {
    try {
        ...
        if (companyCode) query.companyCode = companyCode;
        const item = await Rak.findOne(query);
...
router.get("/:id", async (req, res) => {
    try {
        const item = await Rak.findById(req.params.id);
...
router.post("/", async (req, res) => {
    try {
        ...
        if (data.kode) {
            ...
            const existing = await Rak.findOne(dupQuery);
            if (existing) {
        ...
        const item = await Rak.create(data);
        res.status(201).json(item);
    } catch (err) {
        res.status(400).json({ error: formatError(err, "Rak") });
    }
});

router.put("/:id", async (req, res) => {
    try {
        const existing = await Rak.findById(req.params.id);
        if (!existing) return res.status(404).json({ error: "Not found" });
        if (!checkCompany(existing, req)) return res.status(404).json({ error: "Not found" });
        const item = await Rak.findByIdAndUpdate(req.params.id, req.body, { new: true });
        res.json(item);
    } catch (err) {
        res.status(400).json({ error: formatError(err, "Rak") });
    }
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        if (!existing) return res.status(404).json({ error: "Not found" });
        if (!checkCompany(existing, req)) return res.status(404).json({ error: "Not found" });
        await Rak.findByIdAndDelete(req.params.id);
